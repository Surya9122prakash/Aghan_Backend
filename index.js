...
// Add UPI
app.post('/add-upi', authenticateToken, authorizeAdmin, async (req, res) => {
    const client = await pgPool.connect();
    try {
        const { upi_address, upi_image } = req.body;

        if (!upi_address || !upi_image) {
            return res.status(400).json({ error: 'UPI address and image (base64) are required' });
        }

        const base64Data = new Buffer.from(upi_image.replace(/^data:image\/\w+;base64,/, ""), 'base64');
        const imageType = upi_image.split(';')[0].split('/')[1]; // Extract image type (e.g., "png", "jpeg")
        const params = {
            Bucket: 'aghan', // Replace with your S3 bucket name
            Key: `${uuidv4()}.${imageType}`, // Unique filename (use UUID)
            Body: base64Data,
        ...
        // Insert into the 'upis' table
        const result = await client.query(
            'INSERT INTO upis (upi_id, upi_address, image_name) VALUES ($1, $2, $3) RETURNING upi_id',
            [upi_id, upi_address, imageUrl]
        );

        // Check if insertion was successful by checking the returned upi_id
        ...
        res.status(500).json({ error: 'Failed to add UPI: ' + error.message }); // More specific error message
    } finally {
        client.release(); // Ensure the client is released back to the pool
    }
});
...
app.put('/edit-upi/:upi_id', authenticateToken, authorizeAdmin, async (req, res) => {
    const client = await pgPool.connect();
    try {
        ...
        if (upi_image && upi_image.startsWith('data:')) { // If a new image is provided
            const base64Data = Buffer.from(upi_image.replace(/^data:image\/\w+;base64,/, ""), 'base64');
            const imageType = upi_image.split(';')[0].split('/')[1];
            const params = {
                Bucket: 'aghan', // Replace with your bucket name
                Key: `${uuidv4()}.${imageType}`,
                Body: base64Data,
                ContentEncoding: 'base64',
                ContentType: `image/${imageType}`
            };
        ...
        const values = [upi_address]; // Start with upi_address in values

        if (imageUrl) {  // Add image update to the query if a new image was uploaded
            sql += ', image_name = $2';
            values.push(imageUrl); // Push imageUrl as the second parameter
        }

        sql += ' WHERE upi_id = $' + (values.length + 1); // Dynamically set the placeholder for upi_id
        values.push(upi_id); // Push upi_id as the last parameter
    ...
    try {
        ...
        try {  // Nested try...catch block for the transaction
            ...
            if (company_logo && company_logo.startsWith('data:')) {
                const base64Data = company_logo.replace(/^data:image\/\w+;base64,/, "");
                const imageType = company_logo.split(';')[0].split('/')[1];
                const params = {
                    Bucket: 'aghan',
                    Key: `company-logos/${companyId}.${imageType}`,
                ...
                companyLogoUrl = s3UploadResult.Location;
            } else {
...
app.put('/packages/:package_id', authenticateToken, authorizeAdmin, async (req, res) => {
    const client = await pgPool.connect();
    try {
        ...
        let imageUrl = null;
        if (package_image && package_image.startsWith('data:')) {
            const base64Data = package_image.replace(/^data:image\/\w+;base64,/, "");
            const imageType = package_image.split(';')[0].split('/')[1];
            const params = {
                Bucket: process.env.AWS_BUCKET_NAME,
                Key: `package-images/${package_id}.${imageType}`,
                Body: Buffer.from(base64Data, 'base64'),
                ContentEncoding: 'base64',
                ContentType: `image/${imageType}`
            };
        ...
        if (imageUrl) { // Only update the image if a new one is provided
            query += `, package_image = $9`;
            values.push(imageUrl);
        }
        query += ` WHERE package_id = $10`;
...
app.post('/profile-image-upload', authenticateToken, async (req, res) => {
    const client = await pgPool.connect();
    try {
        const userId = req.user.userId;
        const { profile_image } = req.body; // Get the base64 encoded image from the request

        // Input validation
        if (!profile_image || !profile_image.startsWith('data:')) {
            return res.status(400).json({ error: 'Invalid profile image data. Base64 encoded image required.' });
        }

        // Extract image data and type
        const base64Data = profile_image.replace(/^data:image\/\w+;base64,/, "");
        const imageType = profile_image.split(';')[0].split('/')[1];
        ...
        const params = {
            ...
            Key: `user-profile-images/${fileName}`, // Store images in a subfolder
            Body: Buffer.from(base64Data, 'base64'),
            ContentEncoding: 'base64',
            ContentType: `image/${imageType}`
        };
...
app.post('/upi-details', authenticateToken, async (req, res) => {
    const client = await pgPool.connect();
    try {
        const userId = req.user.userId; // Get userId from authentication middleware
        const { upi_address, googlepay, phonpe, paytm, upi_image } = req.body;
        ...
        try {
            ...
            let imageUrl = null;
            if (upi_image && upi_image.startsWith('data:')) {
                const base64Data = upi_image.replace(/^data:image\/\w+;base64,/, "");
                const imageType = upi_image.split(';')[0].split('/')[1];
                const params = {
                    Bucket: 'aghan', // Your S3 bucket name from environment variables
                    Key: `user-upi-images/${uuidv4()}.${imageType}`,
                    Body: Buffer.from(base64Data, 'base64'),
                    ContentEncoding: 'base64',
                    ContentType: `image/${imageType}`
                };
                try {
                    ...
                    throw new Error('Failed to upload UPI image');
                }
            }
